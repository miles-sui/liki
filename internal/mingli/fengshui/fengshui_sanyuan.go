...
// AllSanYuanYun returns the full 9-period table for a complete 180-year cycle.
func AllSanYuanYun() [9]SanYuanYun {
	var table [9]SanYuanYun
	for i := 0; i < 9; i++ {
		midYear := baseYear + i*periodLength + 10
		table[i] = ComputeSanYuanYun(midYear)
	}
	return table
}
