...
func makeChart(year, month, day, hour, minute int, lon, tz float64, gender ganzhi.Gender) Chart {
	st := tianwen.ComputeSolarTime(year, month, day, hour, minute, lon, tz)
...
func TestShenSha_RealChart(t *testing.T) {
	...
	// 验证四柱
	if cr.Year.Gan != ganzhi.GanJia || cr.Year.Zhi != ganzhi.ZhiZi {
		t.Errorf("年柱 = %s%s, want 甲子", ganzhi.GanName(cr.Year.Gan), ganzhi.ZhiName(cr.Year.Zhi))
	}
	if cr.Month.Gan != ganzhi.GanBing || cr.Month.Zhi != ganzhi.ZhiYin {
		t.Errorf("月柱 = %s%s, want 丙寅", ganzhi.GanName(cr.Month.Gan), ganzhi.ZhiName(cr.Month.Zhi))
	}
	if cr.Day.Gan != ganzhi.GanJi || cr.Day.Zhi != ganzhi.ZhiMao {
		t.Errorf("日柱 = %s%s, want 己卯", ganzhi.GanName(cr.Day.Gan), ganzhi.ZhiName(cr.Day.Zhi))
	}
	if cr.Hour.Gan != ganzhi.GanWu || cr.Hour.Zhi != ganzhi.ZhiChen {
		t.Errorf("时柱 = %s%s, want 戊辰", ganzhi.GanName(cr.Hour.Gan), ganzhi.ZhiName(cr.Hour.Zhi))
	}
	if cr.DayMaster != ganzhi.GanJi {
		t.Errorf("日主 = %s, want 己", ganzhi.GanName(cr.DayMaster))
	}
	...
	for i, wantNames := range expected {
		...
		switch i {
		case 0:
			got = collectShenShaNames(cr.Year)
		case 1:
			got = collectShenShaNames(cr.Month)
		case 2:
			got = collectShenShaNames(cr.Day)
		case 3:
			got = collectShenShaNames(cr.Hour)
		}
