...
func computeFullChart(g chartSnapshot) Chart {
	st := tianwen.ComputeSolarTime(g.Year, g.Month, g.Day, g.Hour, g.Minute, g.Lon, g.TZ*15)
...
// TestGoldenChart_Pillars verifies all pillar ganzhi.Gan/ganzhi.Zhi are valid and DayMaster is set.
...
func TestGoldenChart_JSONSnapshot(t *testing.T) {
	for _, g := range goldenCharts {
		t.Run(g.Name, func(t *testing.T) {
			...
			// nianzhu is a pillarInfo object with gan/zhi/nayin etc.
			if nz, ok := m["nianzhu"].(map[string]any); !ok || nz == nil {
				t.Errorf("nianzhu = %v, want non-nil object", m["nianzhu"])
			}
			if dm, ok := m["riyuan"].(string); !ok || dm == "" {
				t.Error("riyuan must be non-empty")
			}
		})
	}
}
...
func TestGoldenChart_DaYun(t *testing.T) {
	for _, g := range goldenCharts {
		t.Run(g.Name, func(t *testing.T) {
			...
			if cr.DaYun.Direction != "forward" && cr.DaYun.Direction != "backward" {
				t.Errorf("Direction = %q, want forward or backward", cr.DaYun.Direction)
			}
			if cr.DaYun.StartAge < 0 || cr.DaYun.StartAge > 12 {
				t.Errorf("StartAge = %d, want [0,12]", cr.DaYun.StartAge)
			}
			if len(cr.DaYun.Pillars) < 8 {
				t.Errorf("len(Pillars) = %d, want >= 8", len(cr.DaYun.Pillars))
			}
			for i, p := range cr.DaYun.Pillars {
				if p.Gan < 1 || p.Gan > 10 {
...
func TestGoldenChart_NaYin(t *testing.T) {
	for _, g := range goldenCharts {
		t.Run(g.Name, func(t *testing.T) {
			...
			if cr.Year.NaYin == "" {
				t.Error("YearPillar.NaYin is empty")
			}
			if cr.Month.NaYin == "" {
				t.Error("MonthPillar.NaYin is empty")
			}
			if cr.Day.NaYin == "" {
				t.Error("DayPillar.NaYin is empty")
			}
			if cr.Hour.NaYin == "" {
				t.Error("HourPillar.NaYin is empty")
			}
...
func TestGoldenChart_TenGods(t *testing.T) {
	for _, g := range goldenCharts {
		t.Run(g.Name, func(t *testing.T) {
			...
			pillars := [4]pillarInfo{cr.Year, cr.Month, cr.Day, cr.Hour}
			names := [4]string{"year", "month", "day", "hour"}
