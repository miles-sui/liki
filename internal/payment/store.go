...
func (s *Store) MarkPaidIdempotent(ctx context.Context, orderID, paymentID string) (bool, string, product.Product, error) {
	return s.markPaid(ctx, orderID, paymentID)
}
...
func (s *Store) MarkPaidViaCallback(ctx context.Context, orderID string) (newPayment bool, email string, prod product.Product, err error) {
	return s.markPaid(ctx, orderID, "")
}

func (s *Store) markPaid(ctx context.Context, orderID, paymentID string) (bool, string, product.Product, error) {
	var e sql.NullString
	var p string
	query := `UPDATE orders SET status = 'paid', chat_expires_at = COALESCE(NULLIF(chat_expires_at,''), datetime('now', '+7 days')), updated_at = datetime('now') WHERE order_id = ? AND status = 'pending' RETURNING email, product`
	var err error
	if paymentID != "" {
		...
		err = s.db.QueryRowContext(ctx, query, orderID).Scan(&e, &p)
	}
	if err == nil {
	...
	return s.lookupPaidOrder(ctx, orderID)
}

// lookupPaidOrder reads email and product for an already-paid order.
func (s *Store) lookupPaidOrder(ctx context.Context, orderID string) (bool, string, product.Product, error) {
	var e sql.NullString
	var p string
	err := s.db.QueryRowContext(ctx,
