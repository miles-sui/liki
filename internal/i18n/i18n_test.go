...
func TestT_KnownKey(t *testing.T) {
	if got := T(ZH, "err.order_not_found"); got != "订单未找到" {
		t.Errorf("T(ZH) = %q, want 订单未找到", got)
	}
	if got := T(HK, "err.order_not_found"); got != "訂單未找到" {
		t.Errorf("T(HK) = %q, want 訂單未找到", got)
	}
...
func TestT_UnknownKey(t *testing.T) {
	if got := T(ZH, "nonexistent"); got != "nonexistent" {
		t.Errorf("T(unknown) = %q, want key itself", got)
	}
}

func TestT_FallbackToHK(t *testing.T) {
	// Lang("fr") is not in the messages map, so fallback to HK.
...
func TestDetectLang_Empty(t *testing.T) {
	...
	if got := DetectLang(r); got != HK {
		t.Errorf("DetectLang(empty) = %q, want hk", got)
	}
}
...
func TestDetectLang(t *testing.T) {
	tests := []struct {
		...
		{"zh-SG", "zh-SG,zh;q=0.9", ZH},
		{"zh-TW", "zh-TW,zh;q=0.9", HK},
		{"zh-HK", "zh-HK,zh;q=0.9", HK},
		{"zh-Hant", "zh-Hant,zh;q=0.9", HK},
		{"zh", "zh", HK},
		{"en-US", "en-US,en;q=0.9", EN},
		{"en", "en", EN},
		{"zh_with_quality", "zh-CN;q=0.9,en;q=0.5", ZH},
	}
