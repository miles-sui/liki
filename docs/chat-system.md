...
Chat 子系统涉及两类 prompt，分工明确：

- **系统 prompt**（`data/prompts/chat.txt`，嵌入 `doc.ChatPrompt`）：ChatAgent 对话专用。每次 Chat 调用作为 system message 注入。定义产品检测、参数收集、时辰降级等对话行为规则。不包含报告格式。
- **报告模板**（`web/skills/report-*.md`，嵌入 `doc.*ReportPrompt`）：`GenerateFromData()` 专用。支付后 webhook 触发，LLM 按模板将 engine 计算结果展开为完整报告。同时通过 `/skills/report-*.md` 对外公开，供外部 AI agent 参考。

详见 `docs/architecture.md` 提示词体系章节。
