...
| payment_id | TEXT | 可空 | Dodo 支付网关返回的支付 ID |
...
| 操作 | 方法 | SQL |
|------|------|-----|
| 创建订单 | CreateOrder | `INSERT INTO orders (order_id, product, amount, currency, chart_json, llm_json) VALUES (?, ?, ?, ?, ?, ?)` |
| 查询订单 | GetOrder | `SELECT order_id, product, amount, currency, email, chart_json, llm_json, status, COALESCE(payment_id,''), created_at, updated_at FROM orders WHERE order_id = ?` |
| 更新邮箱 | UpdateEmail | `UPDATE orders SET email = ?, updated_at = datetime('now') WHERE order_id = ?` |
| 标记已付(幂等) | MarkPaidIdempotent | 先 UPDATE pending→paid，已 paid 则 SELECT 返回 (幂等，支持 webhook 重发) |
