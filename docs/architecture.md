...
工具和 HTTP 的参数定义统一在 `openapi.json`（OpenAPI 3.0，v1.1.0）。Agent 的 5 个 tool（`get_city_coords`、`compute_chart`、`compute_bond`、`compute_naming`、`purchase`）的 JSON Schema 从 OpenAPI 提取，编译时嵌入，运行时传给 LLM 的 tool calling `parameters` 字段。`openapi.json` 同时包含全部 28 个端点的完整响应 schema（通过 `$ref` 引用 ~80 个 component schema），供外部 AI agent 服务发现使用。
