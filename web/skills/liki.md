...
**路径**：默认走三才路径。用户明确说"只看命理/不信五格"时走命理路径。

---

**命理路径**（只看五行匹配，不看笔画组合和三才五格）：

1. `qiming.sancai` — 取全部 combos（不做筛选，全量传入 compose）
2. `qiming.chars` — 调一次 yong_shen、一次 xi_shen（若传了喜神），不限笔画
3. `qiming.compose` — 传入全部 combos + 全部 chars。禁止自行添加字。
4. 筛选 compose 结果（按选字原则挑 8 个）
5. `qiming.evaluate` — 传入 names + yong_shen + xi_shen + ji_shen。只看 wuxing 三 bool，不看五格三才。

---

**三才路径**（默认，五行+三才五格全维度）：

1. `qiming.sancai` — 取全部 combos
2. 筛选组合：
   a. 剔除笔画差 > 15 的组合
   b. 对比用用与用喜。考察三点：用神的十神角色（印星需比劫转化，比劫直接）、日主强弱（极弱先固本，有根基求流通）、流通路径。不设固定优先级，必须写出八字依据。
3. `qiming.chars` — 按选定的 combos 限定笔画范围，分别调用 yong_shen 和 xi_shen
4. **过滤字库**（在 chars 返回的结构上操作，只删不增）：
   a. 机械删除 — 偏旁含疒/虫/歹/尸、工具器物化学虚词负面动作、生僻字
   b. 语义判断 — 对剩余字逐个问：认识吗？含义积极吗？能举出用这个字的人名吗？任一为"否"就删除
   保留龙/凤/鹤/燕/麟等传统入名吉祥动物字。**禁止脱离 chars 数据结构自行编造字库。**
5. `qiming.compose` — 传入筛选后的 combos + 过滤后的 chars。禁止自行添加字。
6. 筛选 compose 结果 — 剔除含义不雅、声调拗口的名字。按选字原则挑 8 个。
7. `qiming.evaluate` — 传入 names
8. **终检** — 逐名检查五格，**排除天格**，人/地/外/总四格任一为"凶"则剔除，从 compose 结果中另选替换，直到 8 个全通过。
...
用户自选名字时直接调 `qiming.evaluate`，传入用神/喜神/忌神，返回 wuxing + 五格 + 三才全量判定。

**报告**：evaluate 完成后 → 读取 https://liki.hk/skills/report-naming.md，按其模板格式输出。
...
- 1.5.0: 起名路径重构为命理/三才双路径。API 解耦：wuge 拆为 sancai+chars，detail 吞并入 evaluate（支持批量+wuxing三bool）。文档先行，引擎改造待后续。
